...
public final class HopperBoostListener implements Listener {
    ...
    public void onMove(InventoryMoveItemEvent event) {
        ...
        // Only boost real hopper BLOCKS. Hopper minecarts and virtual inventories stay vanilla.
        InventoryHolder holder = event.getInitiator().getHolder();
        if (!(holder instanceof org.bukkit.block.Hopper hopperBlock)) return;

        World world = hopperBlock.getWorld();
    ...
    /** Which source slots may be pulled from. */
    private int[] sourceSlots(Inventory src) {
        InventoryHolder h = src.getHolder();
        if (enforceFurnace && h instanceof Furnace) return new int[]{RESULT}; // finished product only
        if (h instanceof BrewingStand) return new int[]{0, 1, 2};             // finished potions
        return ALL;
    }
    ...
    private int[] destSlots(Inventory src, Inventory dst, ItemStack template) {
        InventoryHolder h = dst.getHolder();
        if (enforceFurnace && h instanceof Furnace) {
            BlockFace facing = BlockFace.DOWN;
            if (src.getHolder() instanceof org.bukkit.block.Hopper hop) facing = hopperFacing(hop);
            if (facing == BlockFace.DOWN) return new int[]{INPUT};   // hopper on top -> smelting slot
            if (template.getType().isFuel()) return new int[]{FUEL}; // hopper on side -> fuel slot
            return null;                                             // side hopper, non-fuel item -> nothing
        }
        if (h instanceof BrewingStand) {
